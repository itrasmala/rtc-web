document.addEventListener('DOMContentLoaded', () => {
  ...
  const EN = {
    ...
    'hero.title': 'Your Trusted Partner<br>in Construction <span class="accent-word">&amp; Contracting</span>',
    'hero.sub': 'We build a future that combines achievement with contracting strength — from design to delivery, backed by more than a decade of electrical and engineering expertise.',
    ...
    'sol.c3.p': 'Contracting companies play a vital role in realizing Saudi Arabia\u2019s Vision 2030.',
    'sol.c3.l1': 'Developing infrastructure and economic and social projects',
  ...
  function applyLang(lang) {
    ...
    document.querySelectorAll('.portfolio-item').forEach((item, i) => {
      ...
      if (label) label.textContent = MESSAGES[lang].project(i + 1);
      if (img) img.alt = MESSAGES[lang].projectAlt(i + 1);
    });
  ...
  for (let i = 1; i <= 30; i++) {
    ...
    item.innerHTML = `<img src="${src}" alt="${MESSAGES.ar.projectAlt(i)}" loading="lazy"><span class="p-label">${MESSAGES.ar.project(i)}</span>`;
  ...
  function openLightbox(index) {
    ...
    lightboxImg.alt = MESSAGES[currentLang].projectAlt(currentIndex + 1);
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'ArrowRight') openLightbox(currentIndex + (rtl ? -1 : 1));
    if (e.key === 'ArrowLeft') openLightbox(currentIndex + (rtl ? 1 : -1));
  });
